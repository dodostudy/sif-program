 ...
 *
 * db.json의 기인물 57종을 빠짐없이 한 그룹에 배정한다. 공단 아카이브 갱신(2026-04)으로
 * 기인물 명칭 14종이 바뀌고 3종이 신설됐으므로, 데이터 교체 시 이 맵을 함께 갱신해야
 * 기인물조회 필터와 발전업 위저드 Step1이 비지 않는다.
 *  · '밀폐'는 정적 목록이 없다 — 재해형태 '산소결핍 질식' 레코드에서 동적 추출한다.
 */
const WORK_TYPE_MAP = {
  '고소': ['비계', '달비계', '이동식비계', '말비계', '사다리', '작업발판/가설계단', '리프트', '슬라브단부', '바닥개구부(자재인양구 등)', '기타 개구부', '엘리베이터피트', '철골구조물', '철골자재', '지붕(처마,홈통)', '지붕 채광판 등(선라이트, 슬레이트)', '천장(판넬 등 마감재)', '거푸집', '작업발판 일체형 거푸집', '동바리'],
  '중장비': ['굴착기(백호우)', '트럭류', '이동식크레인', '타워크레인', '콘크리트펌프카', '지게차', '천공기/항타기', '로울러', '콘크리트믹서', '기타 건설장비', '기타 건설기계기구', '고소작업대(차)'],
  '굴착': ['토사', '흙막이가시설', '암석', '기초파일'],
  '전기': ['충전부', '전주, 철탑'],
  '중량물': ['철근콘크리트자재(철근,콘크리트,PC)', '각재(파이프,강관,합판)', '배관', '줄걸이 기구'],
  '화기': ['용접(단)기', '절단기/톱'],
  '밀폐': null,
  '일반': ['기계설비', '외부마감재', '벽체', '기타 구조물', '기타 가설구조물', '기타 자재(내부마감재,시멘트,마대)', '수목', '방망', '바닥(바닥에 걸려 넘어짐)', '계단', '분류불능', '물'],
  '원인미상': ['원인미상'],
  '화학물질': ['화학물질'],
};
